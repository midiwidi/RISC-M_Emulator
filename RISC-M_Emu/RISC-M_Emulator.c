...
int main(int argc, char *argv[])
{
	...
	// BootROM (MemSlot 0)
	if ((err = create_memory(&RISC->code_memory, 0x0000000, MAIN_SW_START_ADDR-1, 2, 0, RO, "d:\\meine_Software\\RISC-M\\MagRISC_test\\RISC-M_EMU_Test.bin", 0x0000000, BIG_ENDIAN)))
		goto Cleanup;
	// PROM (MemSlot 0)
	if ((err = create_memory(&RISC->code_memory, MAIN_SW_START_ADDR, MAIN_SW_START_ADDR+MAIN_SW_MEM_SIZE-1, 2, 0, RO, "d:\\meine_Software\\RISC-M\\MagRISC_test\\MagRISC_test_PROM.bin", MAIN_SW_START_ADDR, BIG_ENDIAN)))
		goto Cleanup;
	// PRAM (MemSlot 1)
	if ((err = create_memory(&RISC->code_memory, MAIN_SW_START_ADDR, MAIN_SW_START_ADDR+MAIN_SW_MEM_SIZE-1, 2, 1, RO, "d:\\meine_Software\\RISC-M\\MagRISC_test\\MagRISC_test_PRAM.bin", MAIN_SW_START_ADDR, BIG_ENDIAN)))
		goto Cleanup;
	...
	// Create data memory
	if ((err = create_memory(&RISC->data_memory, 0x0000000, DATA_MEM_SIZE-1, 4, 0, RW, NULL, 0x0000000, LITTLE_ENDIAN)))
		goto Cleanup;
